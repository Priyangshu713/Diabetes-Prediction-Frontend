...
export default function GlucoseCard() {
  ...
  const percentage = ((glucoseLevel - 40) / (200 - 40)) * 100;
  ...
  return (
    <Card className="p-6">
      ...
      <div className="cursor-pointer rounded-lg p-2 transition-colors">
        <div className="text-2xl font-bold text-white mb-4">{glucoseLevel} mg/dL</div>

        {/* Glucose Meter with Wave Effect */}
        <div className="relative h-24 mb-6">
          <div className="absolute inset-0 rounded-lg overflow-hidden bg-gray-800/30">
            <div 
        ...
        <div className={`text-sm ${status.color} text-center mt-4`}>{status.text}</div>
      </div>
    </Card>
  );
}